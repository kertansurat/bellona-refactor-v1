/* BELLONA Refactor v2.0 - extracted from production index.html inline script. Logic untouched. */
/* =========================================================
...
function getSidebarPlayerState(p) {
    ...
    if (p.partyId !== null && p.partyId !== undefined) return { text: 'อยู่ในปาร์ตี้แล้ว', cls: 'border-slate-700 bg-slate-900/40 text-slate-400 opacity-70', locked: true };
...
function createPartyCardHTML(index, partyId, prefix) {
    ...
    card.innerHTML = `<div class="flex justify-between items-start gap-2 pb-1.5 border-b border-[#1f2838]">
        <span class="${titleClass} font-extrabold">${prefix} ${index}</span>
    ...
    </div><div class="space-y-1.5">${slots}</div>`;
    return card;
}
...
function renderParties() {
    ...
    assigned.forEach(p => {
        ...
        powerTotals[p.partyId] = (powerTotals[p.partyId] || 0) + getPlayerPowerValue(p);
